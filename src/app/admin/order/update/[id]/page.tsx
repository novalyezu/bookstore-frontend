...
import { IOrderPayload, OrderStatus, useOrderAdmin } from "@/hooks/orderAdmin";
...
export default function UpdateOrder() {
  const { isLoadingPost, order, getOrderById, updateOrder } = useOrderAdmin()
  ...
  const handleClickSubmit = () => {
    ...
    if (isValid) {
      updateOrder(params.id as string, orderPayload)
    }
  }
  ...
  useEffect(() => {
    if (order) {
      ...
      setOrderPayload({
        ...
        carts: order.order_items.map(item => {
          return {
      ...
      setCarts(order.order_items.map(item => {
        return {
          book_id: item.book_id,
          quantity: item.quantity,
          book: {
  ...
  return (
    <div>
      <div className="text-2xl mb-4">Update Order</div>
      ...
      <div className="flex">
        <div className="flex flex-col mb-4 basis-1/2">
          ...
          <input className="w-96 p-2 border-2 rounded-md" type="text" name="buyer_id" id="buyer_id"
            value={orderPayload.buyer_id} onChange={(e) => onChangeInput(e, 'buyer_id')} />
      ...
      <div className="flex">
        <div className="flex flex-col mb-4 basis-1/2">
          ...
          <input className="w-96 p-2 border-2 rounded-md" type="text" name="shipping_name" id="shipping_name"
            value={orderPayload.shipping_name} onChange={(e) => onChangeInput(e, 'shipping_name')} />
        ...
        <div className="flex flex-col mb-4 basis-1/2">
          ...
          <input className="w-96 p-2 border-2 rounded-md" type="text" name="shipping_phone" id="shipping_phone"
            value={orderPayload.shipping_phone} onChange={(e) => onChangeInput(e, 'shipping_phone')} />
          {isSubmit && !orderPayload.shipping_phone && <span className="text-sm text-red-500">shipping phone is required</span>}
        </div>
      </div>
      <div className="flex flex-col mb-4">
        <label htmlFor="shipping_address">Shipping Address *</label>
        <textarea className="p-2 border-2 rounded-md" name="shipping_address" id="shipping_address"
          value={orderPayload.shipping_address} cols={30} rows={10} onChange={(e) => onChangeTextarea(e, 'shipping_address')}></textarea>
